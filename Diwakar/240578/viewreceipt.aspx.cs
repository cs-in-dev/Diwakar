    ...
    public partial class viewreceipt : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (date1.Text != "" && date2.Text != "" && txtMemberID.Text == "" && txtOrderID.Text == "")
                ...
                cmd.CommandText = "SELECT [OrderID], ('OrderDetails2.aspx?ID='+cast(OrderID as nvarchar)) as ViewDetails, [Date], [MemberID], (select UserName from tblMemberMaster where UserCode=[MemberID]) [MemberName], 'UpdateOrders.aspx?ID='+cast([OrderId] as nvarchar) [UpdateOrder], (select isnull(sum(TotalDP),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalDP, (select isnull(sum(TotalBV),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalBV FROM OrderMaster where FrenchiseID = 'GTG250251'  And InvoiceType='DP'  AND OrderId=" + txtOrderID.Text;
            else if (txtOrderID.Text != "" && date1.Text == "" && date2.Text == "" && txtMemberID.Text != "")
                cmd.CommandText = "SELECT [OrderID], ('OrderDetails2.aspx?ID='+cast(OrderID as nvarchar)) as ViewDetails, [Date], [MemberID], (select UserName from tblMemberMaster where UserCode=[MemberID]) [MemberName], 'UpdateOrders.aspx?ID='+cast([OrderId] as nvarchar) [UpdateOrder], (select isnull(sum(TotalDP),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalDP, (select isnull(sum(TotalBV),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalBV FROM OrderMaster where FrenchiseID = 'GTG250251'  And InvoiceType='DP'  AND MemberID='" + txtMemberID.Text + "' and OrderId=" + txtOrderID.Text;
            else if (txtOrderID.Text == "" && date1.Text == "" && date2.Text == "" && txtMemberID.Text != "")
                cmd.CommandText = "SELECT [OrderID], ('OrderDetails2.aspx?ID='+cast(OrderID as nvarchar)) as ViewDetails, [Date], [MemberID], (select UserName from tblMemberMaster where UserCode=[MemberID]) [MemberName], 'UpdateOrders.aspx?ID='+cast([OrderId] as nvarchar) [UpdateOrder], (select isnull(sum(TotalDP),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalDP, (select isnull(sum(TotalBV),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalBV FROM OrderMaster where FrenchiseID = 'GTG250251'  And InvoiceType='DP'  AND MemberID='" + txtMemberID.Text + "'";
            else
                cmd.CommandText = "SELECT [OrderID], ('OrderDetails2.aspx?ID='+cast(OrderID as nvarchar)) as ViewDetails, [Date], [MemberID], (select UserName from tblMemberMaster where UserCode=[MemberID]) [MemberName], 'UpdateOrders.aspx?ID='+cast([OrderId] as nvarchar) [UpdateOrder], (select isnull(sum(TotalDP),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalDP, (select isnull(sum(TotalBV),0) from orderdetails where orderdetails.orderid = OrderMaster.orderid) as TotalBV FROM OrderMaster WHERE FrenchiseID = 'GTG250251'  And InvoiceType='DP'  ORDER BY OrderId ASC";
